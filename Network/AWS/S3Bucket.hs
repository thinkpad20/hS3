...
data VersioningStatus = VersioningDisabled 
                      | VersioningEnabled 
                      | VersioningSuspended
                      deriving (Read, Show, Eq)
...
createBucketWithPrefixIn :: AWSConnection 
                         ...
                         -- ^ Bucket name prefix
                         -> Text 
                         -- ^ Location ("US", "EU", "us-west-1", 
                         -- "ap-southeast-1")
                         -> IO (AWSResult Text) -- ^ Server response, if
                                                --   successful, the bucket
                                                --   name is returned.
createBucketWithPrefixIn aws pre location = do 
  suffix <- randomName
  let name = pre <> "-" <> suffix
  createBucketIn aws name location >>= \case
    Left (AWSError _ _) -> createBucketWithPrefixIn aws pre location
...
createBucketWithPrefix :: AWSConnection -- ^ AWS connection information
                       ...
                       -> IO (AWSResult Text) -- ^ Server response, with bucket name
...
createBucketIn :: AWSConnection -- ^ AWS connection information
             -> Text -- ^ Proposed bucket name
             -> Text -- ^ Location ("US", "EU", "us-west-1", "ap-southeast-1")
...
createBucketIn aws bucket location =
    ...
    in
    do res <- Auth.runAction (S3Action aws bucket "" "" [] (B.pack constraint) PUT)
...
getBucketLocation :: AWSConnection  -- ^ AWS connection information
                  -> Text  -- ^ Bucket name
                  -> IO (AWSResult Text) -- ^ Server response ("US", "EU", "us-west-1", "ap-southeast-1", etc.)
getBucketLocation aws bucket =
    do res <- Auth.runAction (S3Action aws bucket "?location" "" [] B.empty GET)
       case res of
         Left x -> return (Left x)
         Right y -> do bs <- parseBucketLocationXML (B.unpack (rspBody y))
                       return (Right bs)

parseBucketLocationXML :: Text -> IO Text
...
processLocation :: ArrowXml a => a (Data.Tree.NTree.TypeDefs.NTree XNode) Text
...
deleteBucket :: AWSConnection -- ^ AWS connection information
             -> Text -- ^ Bucket name to delete
             -> IO (AWSResult ()) -- ^ Server response
deleteBucket aws bucket =
    do res <- Auth.runAction (S3Action aws bucket "" "" [] B.empty DELETE)
       return (either Left (\_ -> Right ()) res)
...
emptyBucket :: AWSConnection -- ^ AWS connection information
            -> Text -- ^ Bucket name to empty
            -> IO (AWSResult ()) -- ^ Server response
emptyBucket aws bucket =
    do res <- listAllObjects aws bucket (ListRequest "" ""  "" 0)
       let objFromRes x = S3Object bucket (key x) "" [] B.empty
       case res of
         Left x -> return (Left x)
...
listBuckets aws =
    do res <- Auth.runAction (S3Action aws "" "" "" [] B.empty GET)
       case res of
         Left x -> return (Left x)
         Right y -> do bs <- parseBucketListXML (B.unpack (rspBody y))
                       return (Right bs)

parseBucketListXML :: Text -> IO [S3Bucket]
...
listObjects aws bucket lreq =
    do res <- Auth.runAction (S3Action aws bucket ""
                                           ('?' : show lreq) [] B.empty GET)
       case res of
         Left x -> return (Left x)
         Right y -> do let objs = B.unpack (rspBody y)
...
getListResults :: Text -> IO [ListResult]
...
setVersioningConfiguration :: AWSConnection -- ^ AWS connection information
                           -> Text -- ^ Bucket to modify
                           -> VersioningConfiguration -- ^ Desired versioning configuration
                           -> IO (AWSResult ()) -- ^ Server response
setVersioningConfiguration aws bucket vc =
    do res <- Auth.runAction (S3Action aws bucket "" "?versioning" [] (B.pack (versioningConfigurationToXML vc)) PUT)
       case res of
         Left x -> return (Left x)
         Right y -> return (Right ())

versioningConfigurationToXML :: VersioningConfiguration -> Text
...
getVersioningConfiguration :: AWSConnection -- ^ AWS connection information
                           -> Text -- ^ Bucket name to inquire on
                           -> IO (AWSResult VersioningConfiguration) -- ^ Server response
getVersioningConfiguration aws bucket =
    do res <- Auth.runAction (S3Action aws bucket "" "?versioning" [] B.empty GET)
       case res of
         Left x -> return (Left x)
         Right y -> do vc <- parseVersionConfigXML (B.unpack (rspBody y))
                       return (Right vc)

parseVersionConfigXML :: Text -> IO (VersioningConfiguration)
